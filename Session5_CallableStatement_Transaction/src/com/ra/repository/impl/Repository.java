...
public abstract class Repository<E extends IEntity, K> implements IRepository<E, K> {
    ...
    public List<E> findAll() {
        ...
        try {
            ...
            while (rs.next()){
                E entity = clazz.newInstance();  //tạo một đối tượng mới của lớp mà clazz đại diện
                entity.init(rs);   // khởi tạo thông tin từ dữ liệu trong ResultSet
                result.add(entity);
            }
        } catch (Exception ex){
            ex.printStackTrace();
    ...
    @Override
    public boolean remove(Object id) throws SQLException {
        Connection conn = null;
        try {
            conn = MySQLConnect.open();
